...
def upgrade():
    ...
    volume_usage_cache = op.create_table(
        'volume_usage_cache',
        sa.Column('created_at', sa.DateTime(timezone=False)),
        sa.Column('updated_at', sa.DateTime(timezone=False)),
        sa.Column('deleted_at', sa.DateTime(timezone=False)),
    ...
    if bind.engine.name == 'mysql':
        ...
        sa.create_index('usage_id', 'reservations', ['usage_id']),
        sa.create_index(
            'security_group_id',
            'security_group_instance_association',
            ['security_group_id'],
    ...
    with op.batch_alter_table(
        'volume_usage_cache', copy_from=volume_usage_cache,
    ) as batch_op:
    ...
    with op.batch_alter_table(
        'quota_usages', copy_from=quota_usages,
    ) as batch_op:
        batch_op.alter_column('resource', nullable=False)

    with op.batch_alter_table(
        'pci_devices', copy_from=pci_devices,
    ) as batch_op:
        batch_op.alter_column('deleted', nullable=True)
        batch_op.alter_column('product_id', nullable=True)
        batch_op.alter_column('vendor_id', nullable=True)
        batch_op.alter_column('dev_type', nullable=True)
    ...
    # constraint.
    with op.batch_alter_table('key_pairs', copy_from=key_pairs) as batch_op:
        batch_op.drop_constraint('uniq_key_pairs0user_id0name0deleted')
        batch_op.alter_column('name', nullable=False)
        batch_op.create_unique_constraint(
            'uniq_key_pairs0user_id0name0deleted',
            ['user_id', 'name', 'deleted'],
        )
